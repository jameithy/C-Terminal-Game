...
void configure_terminal() { //CHANGE
...
void signal_handler(__attribute__((unused)) int signum) { //AI
    exit_loop = 1;
}
...
int read_key(char* buf, int k) {
    if (buf[k] == '\033' && buf[k + 1] == '[') {
        switch (buf[k + 2]) {
            case 'A': return 1; // Pitch Up
            case 'B': return 2; // Picth Down
            case 'C': return 3; // Roll Left
            case 'D': return 4; // Roll Right
            //case 'F': return 5; // Speed Up
            //case 'K': return 6; // Yaw Left
            //case 'L': return 7; // Yaw Right  
        }
    }
    return 0;
}

void read_input(GameState* state) { // CHANGE AI
...
void handle_player(GameState* state) {//CHANGE
    switch (state->key) {
    case 1:
...
void handle_rocks_gems(GameState* state, int x, int y) {//CHANGE
...
void handle_falling_rocks_gems(GameState* state, int x, int y) {//CHANGE
...
void update_all_elements(GameState* state) {// CHNAGE AI
...
void render(GameState* state) {//CHANGE
        ...
        for (int i = 0; i < MAX_X; ++i) {
            ...
            if (c == '$' || c == 'S') {
                ...
                if (l == 0) {
                    ...
                    printf("\e[48;2;10;10;40m\e[38;2;33;61;255m$");
                }
                continue;
            }
            if (state->old_screen[j][i] != state->screen[j][i]) {
                ...
                case '\n':
                    ...
                    break;
                case '*':
                    printf("\e[48;2;51;51;81m\e[38;2;95;140;77m*");
                    break;
                case '%':
